> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [blog.csdn.net](https://blog.csdn.net/AttaGain/article/details/149826930)
...
*   *   [1、数据关系梳理](#1_7)
    ...
    *   [3、经验总结](#3_68)
*   [三、工具扩展知识](#_72)
*   *   [1、rclone 工具介绍](#1rclone_73)
    *   *   [（1）、rclone 概述](#1rclone_74)
        *   [（2）、安装工具及配置](#2_94)
        *   *   [本地文件迁移到云上服务](#_259)
            *   [本地文件同步到云上服务](#_265)
    *   [2、rsync 工具介绍](#2rsync_272)
    *   *   [（1）、rsync（Remote Sync，远程同步）](#1rsyncRemote_Sync_273)
        *   [（2）rsync 同步源](#2rsync_278)
        *   [（3）、配置 rsync 源](#3rsync_284)
        *   *   [配置源的两种表达方式](#_320)
            *   [免交互格式](#_331)
    *   [3、scp 工具介绍](#3scp_347)
...
上述命令实现数据从 A 服务器[复制到](https://so.csdn.net/so/search?q=%E5%A4%8D%E5%88%B6%E5%88%B0&spm=1001.2101.3001.7020) B 服务器。该命令在执行大文件复制时，可能因为网络环境因素导致数据复制终端，不支持断点续传。
...
采用优化的同步算法，传输前执行压缩,，因此非常适用于异地备份、[镜像服务器](https://so.csdn.net/so/search?q=%E9%95%9C%E5%83%8F%E6%9C%8D%E5%8A%A1%E5%99%A8&spm=1001.2101.3001.7020)等应用。
